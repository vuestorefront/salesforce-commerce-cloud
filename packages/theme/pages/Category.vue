        ...
        <div class="products" v-if="!loading">
          ...
          >
            <SfProductCard
              ...
              @click:wishlist="isInWishlist({ product }) ? removeFromWishlist({ product }) : addItemToWishlist({ product })"
...
import { useCart, useWishlist, productGetters, useFacet, facetGetters } from '@vue-storefront/sfcc';
...
export default {
  ...
  setup(props, context) {
    ...
    const removeFromWishlist = ({ product }) => {
      const item = wishlist.value.items.find((item) => item.productId === product._id);

      if (item) {
        removeItemFromWishlist({ product: item });
      }
    };
